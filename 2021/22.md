...
export default function countDecorations(bigTree) {
	...
	decorations += bigTree.right?.value ?? 0
	decorations += bigTree.left?.value ?? 0

	const branches = [bigTree.left?.left, bigTree.left?.right, bigTree.right?.left, bigTree.right?.right];
	for (const branch of branches) if (branch) decorations += countDecorations(branch);
